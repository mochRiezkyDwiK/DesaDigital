    ...
    try {
      if (isRegister) {
        ...
        if (resData.success || resData.token) {
          ...
          // Pembagi jalur rute dinamis (Senior Architect Route Guard)
          if (roleActual === "ADMIN") {
            navigate("/admin");
          } else {
            // Jika dia WARGA, cek level status administrasinya
            if (statusAkunActual === "VERIFIED") {
              navigate("/dashboard-warga");
            } else {
              // Jika statusnya PENDING_PROFILE, PENDING_VERIFICATION, atau DATA_REJECTED, lempar ke form onboarding
